...
public class BigModelNew extends WebSocketListener {
    ...
    public static final String appid = "2646df92";
    public static final String apiSecret = "MGEzMWNiMzFiM2Q0MDk3M2QzMTk0YWMy";
    public static final String apiKey = "e2378f17383dd4ef66357b489ef139b1";
    ...
    public String doChart(@NotNull String message) {
        // 根据message获取分析需求与数据
        String[] splits = message.split("yyyyy");
        String goal = splits[0];
        String data = splits[1];
        String prompt = String.format("你是一个数据分析师和前端开发专家" +
                "接下来我会按照以下固定格式给你提供内容：分析需求：{%s}" +
                "原始数据：{%s}" +
                "请根据这两部分内容，按照以下指定格式生成内容（此外不要输出任何多余的开头、结尾、注释）" +
                "~~~~~{前端 Echarts V5 的 option 配置对象js代码，合理地将数据进行可视化，不要生成任何多余的内容，比如注释}" +
                "~~~~~{明确的数据分析结论、越详细越好，不要生成多余的注释}", goal, data);
        NewQuestion = prompt;
        // 构建鉴权url
        String authUrl = null;
        try {
            ...
            OkHttpClient client = new OkHttpClient.Builder().build();
            String url = authUrl.toString().replace("ws://", "http://").replace("wss://", "https://");
            Request request = new Request.Builder().url(url).build();
            for (int i = 0; i < 1; i++) {
                totalAnswer = "";
                WebSocket webSocket = client.newWebSocket(request, new BigModelNew(i + "",
